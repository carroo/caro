<div class="content-wrapper">
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0 text-dark" id="title-page">Pengaturan data guru</h1>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Guru Data</h5>
                ...
                <div class="modal-body">
                    <form class="form">
                        ...
                        <div class="form-group">
                            <label>Mapel</label>
                            <select id="mapel" class="form-control" wire:model="form.mapel">
                                <option value="">--Pilih--</option>
                                @foreach ($mapel as $item)
                                    <option value="{{ $item->id }}">--{{ $item->mapel }}--</option>
                                @endforeach
                            </select>
                            @error('form.mapel') <span class="error text-danger">{{ $message }}</span> @enderror
                        </div>
                        <div class="form-group">
                            <label>NIP</label>
                            <input type="number" class="form-control" wire:model='form.nip' placeholder="NIP">
                            @error('form.nip') <span class="error text-danger">{{ $message }}</span> @enderror
                        </div>
                        <div class="form-group">
...
<script>
    $(document).ready(function() {
        ...
        $('#mapel').on('change', function(e) {
            var data = $('#mapel').select2("val");
            @this.set('form.mapel', data);
        });
