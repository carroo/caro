...
class Murid extends Component
{
    ...
    function mount()
    {
        ...
        $this->murid = User::join('jurusan','jurusan.id','=','users.sub_kelas')
                ->Where('level',0)
    ...
    public function carinisn()
    {
        $this->murid =  User::join('jurusan','jurusan.id','=','users.sub_kelas')
                        ->Where('level',0)
                        ->Where('nomer_induk','LIKE',$this->dnisn.'%')
    ...
    public function ckelas()
    {
        if($this->djurusan){
            $this->murid =  User::join('jurusan','jurusan.id','=','users.sub_kelas')
                                ->Where('users.sub_kelas',$this->djurusan)
    ...
    public function liking($id)
    {
        ...
        if($cek){
            Like::where('disuka',$id)->where('penyuka',Auth::user()->id)->delete();
        }else{
            Like::create([
