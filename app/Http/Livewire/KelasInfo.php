...
class KelasInfo extends Component
{
    ...
    public $linkTugas,$link_terkumpul;
    ...
    public function render()
    {
        ...
        return view('livewire.kelas-info',[
            ...
            'murid' => KelasMurid::join('users','users.id','=','kelas_murid.murid_id')
                                ->join('jurusan','jurusan.id','=','users.sub_kelas')
    ...
    public function tugas_info($id)
    {
        ...
        $templinktugas = LinkTugas::where(['murid_id' => Auth::user()->id,'tugas_id' => $this->tugas_info->id])->first();
        if($templinktugas){
            $this->linkTugas = $templinktugas->link_tugas;
        }else{
            $this->linkTugas = NULL;
        }
    }
    public function kirim_link_tugas()
    {
        if($this->linkTugas) {
            LinkTugas::updateOrCreate(
                ['murid_id' => Auth::user()->id,'tugas_id' => $this->tugas_info->id],
                ['link_tugas' => $this->linkTugas]
            );
            $this->dispatchBrowserEvent('swalsuccess','Berhasil ditambahkan');
        }else{
            $this->dispatchBrowserEvent('swalerror','Berhasil ditambahkan');
        }
    }
    public function link_terkumpul($id)
    {
        $this->link_terkumpul = LinkTugas::join('users','users.id','murid_id')
                                            ->where('tugas_id',$id)
                                            ->select('name','link_tugas')
                                            ->get();
    }
}
